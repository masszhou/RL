...
def interact(env, agent, max_steps_episode=300, num_episodes=5000):
    ...
    for i_episode in range(num_episodes):
        ...
        if len(succ_scores) > 0:
            avg_rewards.append(np.mean(succ_scores))
...
if __name__ == "__main__":
    ...
    agent_sarsa = SARSA()
    agent_maxsarsa = MaxSARSA()
    agent_expsarsa = ExpectedSARSA()
    ...
    plt.plot(smooth(np.array(avg_rewards_sarsa), 50), c="r")
    plt.plot(smooth(np.array(avg_rewards_maxsarsa), 50), c="g")
    plt.plot(smooth(np.array(avg_rewards_exppsarsa), 50), c="b")
