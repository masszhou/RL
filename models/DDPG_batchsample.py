...
class DDPG:
    ...
    def act(self, batch_state, add_noise=True):
        batch_state = torch.from_numpy(batch_state).float().to(device)
        self.actor_local.eval()  # must set to eval mode, since BatchNorm used
        with torch.no_grad():
            batch_action = self.actor_local(batch_state).cpu().data.numpy()
        self.actor_local.train()
        if add_noise:
            batch_action += self.noise.sample()
        return np.clip(batch_action, -1, 1)

    def step(self, batch_state, batch_action, batch_reward, batch_next_state, batch_done):
        for sample in zip(batch_state, batch_action, batch_reward, batch_next_state, batch_done):
